...
class BankReconciliationWizard(models.TransientModel):
    ...
    def _do_match(self, line):
        ...
        updated_line = move.line_ids.filtered(lambda ln: ln.account_id == target_line.account_id and not ln.reconciled)[
            :1
        ]
        ...
        if updated_line and not target_line.reconciled:
            ...
            _logger.warning(
                "Rapprochement partiel pour transaction %s : " "lignes déjà réconciliées ou introuvables.", line.id
            )
    ...
    def _get_suspense_lines(self, line, move):
        ...
        if suspense_account:
            suspense_lines = move.line_ids.filtered(lambda ln: ln.account_id == suspense_account)
        else:
            # Fallback : ligne qui n'est pas le compte bancaire
            bank_account = line.journal_id.default_account_id
            suspense_lines = move.line_ids.filtered(lambda ln: ln.account_id != bank_account)
