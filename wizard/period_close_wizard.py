...
class PeriodCloseWizard(models.TransientModel):
            ...
            if not account_benefit:
                ...
                if account_benefit:
                    _logger.warning(
                        ...
                        account_benefit.code,
                        account_benefit.name,
                    )
            if account_benefit:
    ...
    def _generate_opening_entries(self):
        ...
        total_debit = round(sum(ln[2]["debit"] for ln in lines), 2)
        total_credit = round(sum(ln[2]["credit"] for ln in lines), 2)
