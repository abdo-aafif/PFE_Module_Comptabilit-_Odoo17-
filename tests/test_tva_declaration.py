...
class TestTvaDeclaration(TransactionCase):
    ...
    def test_convert_to_company_currency_same_currency(self):
        ...
        result = decl._convert_to_company_currency(1000.0, self.company.currency_id, date(2025, 9, 1))
...
class _TvaComputeCommon(TransactionCase):
    ...
    * Helpers : ``_make_invoice``, ``_pay_invoice``, ``_make_declaration``
    ...
    def _pay_invoice(self, invoice, payment_date, amount=None):
        ...
        ar_ap_line = invoice.line_ids.filtered(
        ...
        pay_line = payment_move.line_ids.filtered(lambda ln: ln.account_id == ar_ap_line.account_id)
...
class TestTvaExportXml(_TvaComputeCommon):
    ...
    def _prepare(self, regime="mensuel", mois="06", trimestre="2", invoice_date=None, payment_date=None):
    ...
    def test_export_nom_fichier_trimestriel(self):
        _, decl = self._prepare(
            regime="trimestriel",
            trimestre="2",
            invoice_date=date(2030, 5, 15),
            payment_date=date(2030, 5, 20),
        )
    ...
    def test_export_xml_structure(self):
        ...
        for tag in ("identifiantFiscal", "annee", "periode", "regime", "releveDeductions"):
            self.assertIsNotNone(root.find(tag), f"Balise <{tag}> absente du XML")
