...
class ResCompany(models.Model):
    ...
    ], string="Fournisseur de Taux", default='floatrates')
        ...
        for company in self:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    active_currencies = self.env['res.currency'].search([('active', '=', True), ('id', '!=', company.currency_id.id)])
                    ...
                    for currency in active_currencies:
                        ...
                        if curr_code in data:
                            ...
                            _logger.info(f"Updated rate for {currency.name}: api_rate={api_rate}, raw_base={raw_base}, db_rate={db_rate}")
...
class AccountFullReconcile(models.Model):
    ...
    def name_get(self):
        result = []
        for rec in self:
            name = rec.name if rec.name and not rec.name.startswith('account.') else str(rec.id)
