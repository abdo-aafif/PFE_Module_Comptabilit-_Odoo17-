...
class CustomFinancialReportLine(models.Model):
    ...
    line_type    = fields.Selection([
        ...
        ('spacer',   'Séparateur'),
    ], string='Type', default='account', required=True)

    account_prefixes = fields.Char(
        string='Préfixes de comptes',
        help='Séparés par des virgules, ex: 711,712,713'
    )
    sign      = fields.Selection([('1', 'Normal (+débit−crédit)'),
                                   ('-1', 'Inversé (crédit−débit)')],
                                  default='-1', string='Signe')
    balance_type = fields.Selection([
        ('cumul',  'Solde cumulé (bilan)'),
        ('period', 'Solde de période (résultat)'),
    ], string='Type de solde', default='period')
...
class CustomReportResultWizard(models.TransientModel):
    ...
    report_id  = fields.Many2one('custom.financial.report', required=True,
                                  string='Rapport')
    company_id = fields.Many2one('res.company', required=True,
                                  default=lambda s: s.env.company)
    date_from  = fields.Date(string='Date début')
    date_to    = fields.Date(string='Date fin', default=fields.Date.context_today,
                              required=True)
    ...
    def action_compute(self):
        ...
        for l in lines:
            l['wizard_id'] = self.id
            self.env['custom.report.result.line'].create(l)
