...
class FinancialStatementWizard(models.TransientModel):
    ...
    def _compute_resultat_net(self, date_from, date_to, company_id):
        ...
        P = lambda p: self._period_balance(p, date_from, date_to, company_id, sign=-1)
        C = lambda p: self._period_balance(p, date_from, date_to, company_id, sign=1)
        produits = P(['711','712','713','714','716','718','719',
                      '732','733','738','739',
                      '751','756','757','758','759'])
        charges = C(['611','612','613','614','616','617','618','619',
                     '631','633','638','639',
                     '651','656','658','659','670'])
    ...
    def _compute_cumulative_resultat(self, date_to, company_id):
        ...
        if last_close:
            ...
            if df > date_to:
                return 0.0
            P = lambda p: self._period_balance(p, df, date_to, company_id, sign=-1)
            C = lambda p: self._period_balance(p, df, date_to, company_id, sign=1)
        else:
            P = lambda p: self._balance_at(p, date_to, company_id, sign=-1)
            C = lambda p: self._balance_at(p, date_to, company_id, sign=1)
    ...
    def _get_bilan_lines(self):
        ...
        def L(name, amt, lvl=1, total=False, bold=False, section='actif'):
            seq[0] += 1
            return dict(name=name, amount=amt or 0.0, level=lvl,
                        is_total=total, bold=bold, section=section,
                        sequence=seq[0], wizard_id=False)

        def bal(p):  return self._balance_at(p, dt, cid,  1)
        def cbal(p): return self._balance_at(p, dt, cid, -1)
        ...
        cap_prop    = (capital + primes + ecarts_reev + res_legale + autres_res
                       + report_nv + res_inst + res_net_ex)
        ...
        t_fin_perm  = (cap_prop + cap_assim + dettes_fin + prov_dur
                       + cpt_liaison + ecart_conv_passif_perm)
    ...
    def _get_cpc_lines(self):
        ...
        def L(name, amt, lvl=1, total=False, bold=False):
            seq[0] += 1
            return dict(name=name, amount=amt or 0.0, level=lvl,
                        is_total=total, bold=bold,
                        sequence=seq[0], wizard_id=False)

        # produits → credit surplus → sign=-1 to get positive
        def P(p): return self._period_balance(p, df, dt, cid, sign=-1)
        # charges  → debit surplus → sign=+1
        def C(p): return self._period_balance(p, df, dt, cid, sign=1)
    ...
    def _get_flux_lines(self):
        ...
        def bat(p):   return self._balance_at(p, dt, cid)
        def bat0(p):  return self._balance_at(p, dt0, cid)
        def delta(p): return bat(p) - bat0(p)
        def P(p):     return self._period_balance(p, df, dt, cid, sign=-1)
        def C(p):     return self._period_balance(p, df, dt, cid, sign=1)
        ...
        flux_expl = (res_net
                     + dotations - reprises
                     + vnc_cessions - prod_cessions   # neutralisation cessions
                     + bfr)
        ...
        immo_brut_prefixes = ['211','212','213',
                              '221','222','223','228',
                              '231','232','233','234','235','238','239',
                              '241','248','251','258','271','272']
