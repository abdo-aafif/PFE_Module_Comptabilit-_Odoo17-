...
class AccountPeriodClose(models.Model):
    ...
    name = fields.Char(
        string='Référence', required=True, readonly=True,
    ...
    date_close = fields.Date(
        string='Date de clôture', default=fields.Date.context_today,
    ...
    opening_move_id = fields.Many2one(
        'account.move', string='Écriture à-nouveaux', readonly=True,
    )
    opening_move_state = fields.Selection(
        related='opening_move_id.state', string='État écriture', readonly=True,
    )
        ...
        for rec in self:
            ...
            if duplicate:
                raise ValidationError(_(
                    "Une clôture validée existe déjà pour cette période (%s)."
                ) % duplicate.name)
    ...
    def action_post_opening_move(self):
        ...
        self.message_post(body=_(
            "Écriture à-nouveaux %s comptabilisée par %s."
    ...
    def action_cancel_opening_move(self):
        ...
        self.message_post(body=_(
            "Écriture à-nouveaux %s supprimée par %s. Vous pouvez la régénérer."
    ...
    def action_regenerate_opening_move(self):
        ...
        if move:
            ...
            self.message_post(body=_(
                "Écriture à-nouveaux régénérée par %s (en brouillon, à comptabiliser)."
            ) % self.env.user.name)
    ...
    def action_unlock(self):
        ...
        if not self.env.user.has_group('account.group_account_manager'):
            raise UserError(_(
                "Seul un Manager Comptable peut déverrouiller une clôture."
            ))
        if self.state != 'done':
            raise UserError(_("Seule une clôture en état 'Clôturée' peut être déverrouillée."))
        if not self.note or not self.note.strip():
            raise UserError(_(
                "Veuillez saisir un motif dans le champ \"Observations\" avant de déverrouiller."
            ))
